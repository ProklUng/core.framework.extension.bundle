 ...
 * @internal
 *
 * @internal Форк из Symfony 5.2 (пришлось пробрасывать контейнер снаружи)
 */
...
EOF
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $options['output'] = $io;
        $options['is_debug'] = $this->container->getParameter('kernel.debug');

        try {
            $helper->describe($io, $object, $options);

            if (isset($options['id']) && isset($this->getContainerBuilder()->getRemovedIds()[$options['id']])) {
     ...
     */
    protected function getContainerBuilder(): ContainerInterface
    {
